...
router.get("/:title", async (req, res) => {
  ...
  $(
    'meta, link[rel="dc:isVersionOf"], link[href*="mediawiki.legacy.commonPrint"], title, base'
  ).remove();
  ...
  const mwBodyDiv = $(
    '<div class="mw-body-content mw-content-ltr" lang="en" dir="ltr"></div>'
  );
  ...
  $(".mw-parser-output h2, .mw-parser-output h3").each((index, element) => {
    ...
    if (tagType === "h2") {
      ...
      tocListElement
        .find(`.toclevel-1.tocsection-${tocLevel1}`)
        .last()
        .append(`<ul>${tocEntryLevel2}</ul>`);
    }
  });
